<?php
    ...
    if (isset($_GET['id']) AND !empty($_GET['id'])) 
    {
        ...
        $secretaire = $obj->Requete("SELECT * FROM user WHERE administration '".$date_paiement['USER']."'")->fetch();
        $montant_payer = $obj->Requete('SELECT SUM(MONTANT) FROM `historique_payement` WHERE ID_INSCRIPTION ='.$old_student['ID_INSCRIPTION'].' AND DATE_PAYEMENT <="'.$date_paiement['DATE_PAYEMENT'].'"')->fetch();
    }
