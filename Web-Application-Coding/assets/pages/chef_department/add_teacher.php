<?php
    ...
    $classes = $obj->Select('classe', array('ID_CLASSE', 'NOM_CLASSE'), [], [], $orderBy = 'NOM_ClASSE', $order = 1);
                                        ...
                                        <div class="col-lg-4">
                                            <div class="form-group my-3">
                                                ...
                                                <select name="grade" id="grade" class="form-control" required>
                                                    <option value="null" selected disabled>Select Grade</option>
                                        ...
                                        <div class="col-lg-4">
                                            <div class="form-group my-3">
                                                ...
                                                <?php 
                                                    ...
                                                    { 
                                                        // $exist = $obj->Requete("SELECT EXISTS( SELECT * FROM module WHERE ID_CLASSE =  ".$class['ID_CLASSE']."} )");
                                                        // $numRows = $obj->Select("module", ["ID_MODULE", "NOM_MODULE"], ["ID_CLASSE"=>$class['ID_CLASSE']]);
                                                        $numRows = $obj->Requete("Select count(*) as x from module where ID_CLASSE = ". $class['ID_CLASSE'])->fetch();
                                                        if($numRows['x']>0)
                                                        {
                                                        ?>
                                                            <optgroup label="<?= $class['NOM_CLASSE'] ?>">
                                                            <?php 
                                                            $modulus = $obj->Select("module", array('NOM_MODULE', 'ID_MODULE'), array('ID_CLASSE'=>$class['ID_CLASSE']), $orderBy = 'NOM_MODULE', $order = 1);
                                                            while($modul = $modulus->fetch())
                                                            {?>
                                                                <option value="<?= $modul['ID_MODULE']?>"><?= $modul["NOM_MODULE"]?></option>
                                                            <?php } ?>
                                                            </optgroup>
                                                   <?php } ?>
