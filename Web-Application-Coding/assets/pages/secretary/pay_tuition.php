<?php 
    ...
    //*******************Incription of new student*********************/
        ...
        if (isset($_POST['submit_student_fees']))
        {
            ...
            $user=$obj->Insert('user',array('PASSWORD','USERNAME','DROITS' , 'PROFILE_PIC' , 'USER_STATUS'),array($inscription_matricule, $inscription_matricule, 'student','user_24px.png','enable'));
            $id_username = $obj->Requete('SELECT ID_USER FROM user order by ID_USER DESC LIMIT 1')->fetch();
               ...
               //die("tttttttttttt");
                ...
                if ($subs_new_student == 1)
                {
                    ...
                    if($last_ins_id->rowCount()>0){
                        ...
                        $last_ins_scolr = $obj->Requete('SELECT ID_INSCRIPTION FROM scolarite ORDER BY ID_SCOLARITE DESC LIMIT 1');
                        $last_ins_scolr_id = $last_ins_scolr->fetch();
                        $last_ins_scolr_id = $last_ins_scolr_id['ID_INSCRIPTION'];
                        ...
                        if($last_ins_scolr->rowCount()>0)
                        {
                            ...
                            $_SESSION['first_pay'] = 1;
                            /*===================redirect to student list page===========================**/
                            Redirect('recu.php?id=' .$inscription_matricule .'&firstname='.$new_student_paying["PRENOM"] .'&lastname='.$new_student_paying["NOM"]);
                        }
                    }
                }
            }
                            ...
                            <form action="" method="post" >
                                <div class="row">
                                    ...
                                    <div class="col-lg-6 p-4">
                                        ...
                                        <div class="input-group">
                                            <input type="text" name="inscription_actu_fees" id="inscription_actu_fees" class="form-control" value="<?= $normal_scholar_fees - ($normal_scholar_fees * $scholar_rate/100).' F CFA' ?>" readonly >
                                            <div class="input-group-append">
                                                <span class="input-group-text fas fa-portrait text-primary">
                                                </span>
                                            </div>
